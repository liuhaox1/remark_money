...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _openFilterSheetOld() async {
    ...
    // 仅针对有父级的叶子分类做推荐，避免「餐饮」这类一级分类出现在常用区造成混淆
    final List<Category> sortedLeafCategories =
        categories.where((c) => c.parentKey != null).toList()
          ..sort(
            (a, b) => (categoryUsage[b.key] ?? 0)
                .compareTo(categoryUsage[a.key] ?? 0),
          );
    // ignore: unused_local_variable
    // 只取使用频率最高的一小部分作为「常用分类」，避免退化为“全部分类”
    final List<Category> recommendedCategories =
        sortedLeafCategories.take(12).toList();

    final Map<String, int> rootUsage = <String, int>{};
    for (final record in allRecords) {
      final key = record.categoryKey;
      if (key.isEmpty) continue;
      ...
      final rootKey = current.key;
      rootUsage.update(rootKey, (value) => value + 1, ifAbsent: () => 1);
    }
    // 读取一次旧的推荐结果，避免未使用告警，同时方便后续调试对比
    if (recommendedCategories.isNotEmpty) {
      // no-op
    }
    final List<Category> recommendedRootCategories =
        categories
            .where((c) => c.parentKey == null)
            .toList()
          ..sort(
            (a, b) =>
                (rootUsage[b.key] ?? 0).compareTo(rootUsage[a.key] ?? 0),
          );
                              ...
                              Builder(
                                builder: (context) {
                                  final recentCategories =
                                      recommendedRootCategories.where((c) {
                                    if (tempIncomeExpense != null) {
                                      if (tempIncomeExpense == true &&
                                          c.isExpense) {
                                        return false;
                                  ...
                                  return Column(
                                    ...
                                    children: [
                                      ...
                                      Wrap(
                                        ...
                                        runSpacing: 8,
                                        children: [
                                          for (final c in recentCategories)
                                            _buildFilterChip(
                                              label: c.name,
                                              selected: tempCategoryKeys
                                                  .contains(c.key),
                                              onSelected: () {
                                                final selected =
                                                    tempCategoryKeys
                                      ...
                                      Align(
                                        ...
                                        child: TextButton.icon(
                                          ...
                                          label: Text(
                                            showAllCategories
                                                ? '${AppStrings.collapse}全部分类'
                                                : '${AppStrings.expand}全部分类',
                                            style: TextStyle(
                                              fontSize: 12,
                                              color: cs.primary,
                                            ),
